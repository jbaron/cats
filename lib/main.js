...
(function (Cats) {
    var Ide = (function (_super) {
        ...
        Ide.prototype.closeSession = function (session) {
            ...
            if(Cats.mainEditor.activeSession === session) {
                Cats.mainEditor.activeSession = null;
    ...
    (function (Commands) {
        ...
        function closeFile() {
            if(Cats.mainEditor.activeSession) {
                Cats.IDE.closeSession(Cats.mainEditor.activeSession);
            }
        }
        function closeAllFiles() {
            var sessions = Cats.IDE.sessions;
            sessions.forEach(function (session) {
                Cats.IDE.closeSession(session);
            });
        }
...
(function (Cats) {
    (function (Commands) {
        ...
        function propertiesProject() {
            ...
            Cats.project.editFile(Cats.ConfigLoader.getFileName(Cats.project.projectDir), content);
        }
...
(function (Cats) {
    (function (Commands) {
        function quit() {
            if(Cats.IDE.hasUnsavedSessions()) {
...
(function (Cats) {
    (function (Commands) {
        ...
        function rename() {
            ...
            while(i--) {
                var data = rows[i];
                var session = Cats.IDE.getSession(data.unitName);
                if(!session) {
            ...
            if(session) {
                session.project.iSense.perform("getFormattedTextForRange", session.name, 0, session.getValue().length, function (err, result) {
                    if(!err) {
                        ...
                        if(pos) {
                            Cats.mainEditor.aceEditor.moveCursorToPosition(pos);
                        }
                    }
                });
            }
...
(function (Cats) {
    (function (Menu) {
        ...
        function initFileContextMenu() {
            ...
            Cats.IDE.fileNavigation.addEventListener('contextmenu', function (ev) {
...
(function (Cats) {
    function setOverwrite() {
        Cats.mainEditor.overwrite = Cats.mainEditor.activeSession.editSession.getOverwrite();
    }
    var Editor = (function (_super) {
        ...
        Editor.prototype.onEditMode = function (fn) {
            this.on("editMode", fn);
        };
        Editor.prototype.onActiveSession = function (fn) {
            this.on("activeSession", fn);
        };
        Editor.prototype.onOverwrite = function (fn) {
        ...
        Editor.prototype.swapListeners = function (oldSession, newSession) {
            var listeners = {
...
(function (Cats) {
    ...
    function initTabBar() {
        ...
        Cats.IDE.on("sessions", function (sessions) {
            Cats.tabbar.setOptions(sessions);
        });
    }
    function initNavBar() {
    ...
    function initOutlineView() {
        Cats.mainEditor.on("activeSession", function (session) {
    ...
    function initStatusBar() {
        ...
        Cats.mainEditor.on("editMode", function (mode) {
            sessionMode.innerText = mode;
        });
        var overwriteMode = document.getElementById("overwritemode");
        Cats.mainEditor.onOverwrite(function (mode) {
    ...
    Cats.project = new Cats.Project(determineProject());
    Cats.Commands.init();
