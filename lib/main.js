...
var SearchDialog = (function (_super) {
    ...
    SearchDialog.prototype.run = function (search, filePattern) {
        var _this = this;
        ...
        if (!filePattern)
            filePattern = "**/*";
        OS.File.find(filePattern, this.rootDir, function (err, files) {
            files.forEach(function (file) {
                try  {
                    var fullName = OS.File.join(_this.rootDir, file);
                    var content = OS.File.readTextFile(fullName);
            ...
            var toolTipText = "Search results for " + searchPattern + " in " + _this.rootDir + "/" + filePattern;
    ...
    SearchDialog.prototype.createForm = function () {
        ...
        searchButton.addListener("execute", function () {
            if (_this.form.validate()) {
                _this.run(s.getValue(), p.getValue());
            }
            ;
        }, this);
