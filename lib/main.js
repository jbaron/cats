...
(function (Cats) {
    var Ide = (function (_super) {
        ...
        function Ide(doc) {
            ...
            infoBus.IDE.on("toggleView", function (name) {
                return _this.layoutConfig.toggle(name);
            });
        }
        ...
        Ide.prototype.getActiveSession = function () {
            var page = this.sessionTabView.getSelection()[0];
            if (!page)
                return null;
            var editor = page.getChildren()[0];
...
(function (Cats) {
    ...
    (function (Commands) {
        ...
        function editorCommand(commandName) {
            return function () {
                IDE.getActiveEditor().execCommand(commandName);
            };
        }
            ...
            if (session) {
                session.project.iSense.getFormattedTextForRange(session.name, 0, session.getValue().length, function (err, result) {
                    if (!err) {
                        ...
                        if (pos)
                            IDE.getActiveEditor().moveCursorToPosition(pos);
                    }
                });
            }
        }
        ...
        var Grid = (function () {
            ...
            Grid.prototype.render = function () {
                ...
                this.rows.forEach(function (row) {
                    var r = _this.createRow(tbody, row);
                    if (_this.onrender)
                        _this.onrender(r, row);
                });
            };
            Grid.DefaultHandler = function (row, name) {
                return row[name];
            };
            return Grid;
        })();
                ...
                this.rootElem.onclick = function (ev) {
                    ...
                    if (elem.tagName.toLowerCase() === "span") {
                        if (_this.onselect) {
                            ...
                            if (entry.isFolder)
                                _this.handleClick(elem.parentNode);
...
var ToolBar = (function (_super) {
    ...
    ToolBar.prototype.___init = function () {
        var _this = this;
...
var SessionTabView = (function (_super) {
    ...
    SessionTabView.prototype.addSession = function (session) {
        ...
        */
        ...
        var Outline = (function (_super) {
            ...
            Outline.prototype.handleOutlineEvent = function (session) {
                ...
                project.iSense.getScriptLexicalStructure(session.name, function (err, data) {
                    /*
