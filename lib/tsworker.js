    ...
    (function (TSWorker) {
        ...
        class ISense {
            ...
            getErrors(fileName) {
                ...
                var result = errors.map(function (error) { return this.convertError(error); });
                return result;
            }
            /**
            ...
            setConfigFile(path, content) {
                var result = ts.readConfigFile(path, function (path) { return content; });
                var options = result.config.compilerOptions;
                this.lsHost.setCompilationSettings(options);
                this.formatOptions = this.getDefaultFormatOptions();
                var editorOptions = options.formatCodeOptions || {};
