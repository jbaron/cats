        ...
        var ISense = (function () {
            ...
            ISense.prototype.getObjectModel = function () {
                ...
                this.lsHost.getScriptFileNames().forEach(function (scriptName) {
                    ...
                    var doc = _this.documentRegistry.acquireDocument(scriptName, _this.lsHost.getCompilationSettings(), script.getScriptSnapshot(), script.getVersion(), script.isOpen());
                    mc.parse(doc);
    ...
    (function (TSWorker) {
        ...
        var ModelCreator = (function () {
            ...
            ModelCreator.prototype.handle = function (node) {
                ...
                try {
                    ...
                    if (node.kind === 125 /* Method */) {
                        if (this.last)
                            this.last.operations.push(node.symbol.name);
                        return;
                    }
            ...
            ModelCreator.prototype.isTopLevelNode = function (kind) {
                return ((kind === 186 /* ClassDeclaration */) || (kind === 187 /* InterfaceDeclaration */) || (kind === 189 /* EnumDeclaration */));
            };
            ...
            ModelCreator.prototype.createIfNotExist = function (node) {
                ...
                if (!this.model[fullName]) {
                    ...
                    if (node.kind === 186 /* ClassDeclaration */) {
                        var cd = node;
                        if (cd.baseType)
                            entry.extends.push(this.getName(cd.baseType));
                        if (cd.implementedTypes)
                            cd.implementedTypes.forEach(function (implType) {
                                entry.implements.push(_this.getName(implType));
                            });
                    }
                    if (node.kind === 187 /* InterfaceDeclaration */) {
                        var id = node;
                        if (id.baseTypes)
                            id.baseTypes.forEach(function (type) {
                                entry.implements.push(_this.getName(type));
                            });
                    }
                }
